from sqlalchemy import Column, Integer, String, Table, ForeignKey
from sqlalchemy.orm import relationship

from models.base import BASE
from models.newspaper import noun_chunks_association

news_association = Table(
    "new_noun_chunk",
    BASE.metadata,
    Column("new_id", Integer, ForeignKey("new.id")),
    Column("noun_chunk_id", Integer, ForeignKey("noun_chunk.id")),
)


class NounChunk(BASE):
    __tablename__ = "noun_chunk"

    id = Column(Integer, primary_key=True)
    value = Column(String(255), unique=True)

    news = relationship("New", secondary=news_association, back_populates="noun_chunks")

    newspapers = relationship("Newspaper", secondary=noun_chunks_association, back_populates="noun_chunks")

    def __iter__(self) -> iter:
        yield "value", self.value
