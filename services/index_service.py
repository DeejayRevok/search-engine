...
class IndexService:
    ...
    async def index_new(self, new: New) -> NewModel:
        ...
        """
        async def _index_source(source_name: str) -> Source:
            """
        ...
        async def _index_new(new_title: str, new_sentiment: float, source: Source) -> NewModel:
            """
            Index the new data
        ...
        with self._app['session_provider'](read_only=False):
            if new.source:
                saved_source_model = await _index_source(new.source)

            if new.title:
                saved_new_model = await _index_new(new.title, new.sentiment, saved_source_model)

            if new.entities:
                await _index_named_entities(new.entities, saved_new_model)
