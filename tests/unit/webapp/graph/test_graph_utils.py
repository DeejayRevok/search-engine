...
class TestGraphUtils(TestCase):
    def test_authenticated_field_success(self):
        ...
        result_connection = AuthenticatedFilterableField.resolve_connection(
            mock_connection, MagicMock(), mock_resolve_info, MagicMock(), MagicMock()
        )
    ...
    def test_authenticated_field_fails(self):
        ...
        with self.assertRaises(HTTPUnauthorized):
            AuthenticatedFilterableField.resolve_connection(
                mock_connection, MagicMock(), mock_resolve_info, MagicMock(), MagicMock()
            )
